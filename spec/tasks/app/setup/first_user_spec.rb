...
describe 'rake app:setup:first_user' do
  ...
  it 'Creates first user' do
    ...
    expect(User).to receive(:any?).and_return(false)
  ...
  it "Doesn't create a second user" do
    ...
    expect(STDOUT).to receive(:puts).with('Create first user skipped - You already have a user in the database!')
    expect { task.execute }.not_to raise_error
  end
